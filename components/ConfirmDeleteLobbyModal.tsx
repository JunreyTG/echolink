...
interface ConfirmDeleteLobbyModalProps {
  room: Room;
  onClose: () => void;
  onConfirm: () => void;
}

const ConfirmDeleteLobbyModal: React.FC<ConfirmDeleteLobbyModalProps> = ({ room, onClose, onConfirm }) => {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={onClose} aria-modal="true" role="dialog">
      <div className="bg-white dark:bg-[#2f3136] rounded-lg shadow-2xl w-full max-w-md mx-4 p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold text-black dark:text-white">Delete Lobby</h2>
        ...
        <p className="text-gray-700 dark:text-gray-300 mb-6">
          Are you sure you want to permanently delete the lobby <span className="font-semibold text-black dark:text-white">"{room.name}"</span>? This action cannot be undone.
        </p>
...
export default ConfirmDeleteLobbyModal;